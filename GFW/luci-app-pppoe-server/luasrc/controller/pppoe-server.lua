...
function index()
    ...
    entry({"admin", "vpn", "pppoe-server"},
          alias("admin", "vpn", "pppoe-server", "settings"),
          _("PPPoE Server"), 3)
    entry({"admin", "vpn", "pppoe-server", "settings"},
          cbi("pppoe-server/settings"), _("General Settings"), 10).leaf = true
    entry({"admin", "vpn", "pppoe-server", "users"},
          cbi("pppoe-server/users"), _("Users Manager"), 20).leaf = true
    entry({"admin", "vpn", "pppoe-server", "online"},
          cbi("pppoe-server/online"), _("Online Users"), 30).leaf = true
    entry({"admin", "vpn", "pppoe-server", "status"}, call("status")).leaf =
        true
end
