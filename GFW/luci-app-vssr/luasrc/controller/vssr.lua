...
function index()
    ...
    if nixio.fs.access('/usr/bin/ssr-redir') then
        entry({'admin', 'vpn', 'vssr'}, alias('admin', 'vpn', 'vssr', 'client'), _('Hello World'), 0).dependent = true -- 首页
        entry({'admin', 'vpn', 'vssr', 'client'}, cbi('vssr/client'), _('SSR Client'), 10).leaf = true -- 基本设置
        entry({'admin', 'vpn', 'vssr', 'servers'}, cbi('vssr/servers'), _('Severs Nodes'), 11).leaf = true -- 服务器节点
        entry({'admin', 'vpn', 'vssr', 'servers'}, arcombine(cbi('vssr/servers'), cbi('vssr/client-config')), _('Severs Nodes'), 11).leaf = true -- 编辑节点
        entry({'admin', 'vpn', 'vssr', 'subscribe_config'}, cbi('vssr/subscribe-config', {hideapplybtn = true, hidesavebtn = true, hideresetbtn = true}), _('Subscribe'), 12).leaf = true -- 订阅设置
        entry({'admin', 'vpn', 'vssr', 'control'}, cbi('vssr/control'), _('Access Control'), 13).leaf = true -- 访问控制
        entry({'admin', 'vpn', 'vssr', 'router'}, cbi('vssr/router'), _('Router Config'), 14).leaf = true -- 访问控制
        if nixio.fs.access('/usr/bin/v2ray/v2ray') or nixio.fs.access('/usr/bin/v2ray') or nixio.fs.access('/usr/bin/xray') or nixio.fs.access('/usr/bin/xray/xray') then
            entry({'admin', 'vpn', 'vssr', 'socks5'}, cbi('vssr/socks5'), _('Local Proxy'), 15).leaf = true -- Socks5代理
        end
        entry({'admin', 'vpn', 'vssr', 'advanced'}, cbi('vssr/advanced'), _('Advanced Settings'), 16).leaf = true -- 高级设置
    ...
    if nixio.fs.access('/usr/bin/ssr-server') then
        entry({'admin', 'vpn', 'vssr', 'server'}, arcombine(cbi('vssr/server'), cbi('vssr/server-config')), _('SSR Server'), 20).leaf = true
    end

    entry({'admin', 'vpn', 'vssr', 'log'}, cbi('vssr/log'), _('Log'), 30).leaf = true
    --entry({'admin', 'vpn', 'vssr', 'licence'}, template('vssr/licence'), _('Licence'), 40).leaf = true
    ...
    entry({'admin', 'vpn', 'vssr', 'switch'}, call('switch')) -- 设置节点为自动切换
    entry({'admin', 'vpn', 'vssr', 'delnode'}, call('del_node')) -- 删除某个节点
end
